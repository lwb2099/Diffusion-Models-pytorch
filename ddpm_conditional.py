...
class Diffusion:
    ...
    def one_epoch(self, train=True):
        ...
        if train:
            ...
            self.model.eval()
        pbar = (
            progress_bar(self.train_dataloader, leave=False)
            if train
            else progress_bar(self.val_dataloader, leave=False)
        )
        for i, (images, labels) in enumerate(pbar):
            # * autocast: mix precision; inference_mode: no grad
            with torch.autocast("cuda") and (
    ...
    def log_images(self):
        ...
        wandb.log(
            {
                "sampled_images": [
                    wandb.Image(img.permute(1, 2, 0).squeeze().cpu().numpy())
    ...
    def load(
        self,
        model_cpkt_path,
        model_ckpt="conditional_ckpt.pt",
        ema_model_ckpt="conditional_ema_ckpt.pt",
    ):
        self.model.load_state_dict(
            torch.load(os.path.join(model_cpkt_path, model_ckpt))
        )
        self.ema_model.load_state_dict(
            torch.load(os.path.join(model_cpkt_path, ema_model_ckpt))
        )
    ...
    def save_model(self, run_name, epoch=-1):
        ...
        torch.save(
            ...
            os.path.join("models", run_name, f"conditional_ckpt.pt"),
        )
        torch.save(
            self.ema_model.state_dict(),
            os.path.join("models", run_name, f"conditional_ema_ckpt.pt"),
        )
        torch.save(
            self.optimizer.state_dict(),
            os.path.join("models", run_name, f"conditional_optim.pt"),
        )
        at = wandb.Artifact(
            "model",
            type="model",
    ...
    def prepare(self, args):
        ...
        if os.path.exists(
            os.path.join(args.model_ckpt_path, "conditional_optimizer.pt")
        ):
    ...
    def fit(self, args):
        if os.path.exists(
            os.path.join(args.model_ckpt_path, "conditional_ckpt.pt")
        ) and os.path.exists(os.path.join(args.model_ckpt_path, "conditional_ckpt.pt")):
            self.load(
...
def parse_args(config):
    ...
    parser.add_argument(
        "--img_size", type=int, default=config.img_size, help="image size"
    )
    parser.add_argument(
        "--num_classes", type=int, default=config.num_classes, help="number of classes"
    )
    parser.add_argument(
        "--dataset_path", type=str, default=config.dataset_path, help="path to dataset"
    )
    ...
    parser.add_argument(
        "--noise_steps", type=int, default=config.noise_steps, help="noise steps"
    )
...
if __name__ == "__main__":
    ...
    diffuser = Diffusion(
        config.noise_steps, img_size=config.img_size, num_classes=config.num_classes
    )
